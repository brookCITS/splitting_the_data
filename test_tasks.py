...
class TestTasks(unittest.TestCase):
    ...
    def test_task_answers(self):
        ...
        for i, task in enumerate(tasks, 1):
            with self.subTest(task=f"Task {i}"):
                ...
                print(f"Task {i}: Answer provided for the question '{question}'.")
    ...
    def test_task3_model_performance(self):
        ...
        test_rmse_threshold = 100.0  # Example threshold, adjust as needed.
    ...
    def test_feedback(self):
        ...
        result1 = task1.run(context)
        if "0.2" in result1["answer"]:
            feedback.append("Good job in observing how different validation splits impact training!")

        # Task 2 feedback example
        result2 = task2.run(context)
        if "training set isn't similar" in result2["answer"].lower():
        ...
        result3 = task3.run(context)
        if "shuffle" in result3["answer"].lower():
